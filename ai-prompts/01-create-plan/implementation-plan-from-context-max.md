...
**Objective:** Based on the design and next steps we just discussed in our preceding conversation regarding **[Completing phases 2 thru 6 for the UI requirements & recommendations and the server api integration implementation plan]**, create a comprehensive, detailed, and prescriptive implementation plan. The plan should contain small, actionable steps, including code templates/patterns and specific guidance where necessary (similar in detail to the RKO example discussed previously), suitable for execution by another AI agent. 
