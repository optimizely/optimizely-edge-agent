/**
 * @fileoverview Forced Variation Tests Adapter
 * 
 * This adapter integrates the original forced variation tests
...
const originalTestPath = path.join(projectRoot, 'final-tests-validation', 'test-scripts', 'forced-variation-tests.js');
...
/**
 * Adapted test function for the forced variation tests
 * @param {object} context - Enhanced test context from environment-aware framework
 * @returns {object} Test result
 */
async function forcedVariationTests(context) {
  // Extract utilities from context
  const { utils, baseUrl } = context;
  
  console.log(`Running Forced Variation Tests in ${context.environmentName} environment`);
  console.log(`Base URL: ${baseUrl}`);
  
  // Test cases for forced variations
  const testCases = [
    {
      name: 'Basic forced variation',
      payload: {
    ...
    {
      name: 'Multiple forced variations',
      payload: {
        sdkKey: '8mR1pGh8u2ztUP8GqjmQq',
        userId: 'test-user-2',
        forceVariation: {
    ...
    {
      name: 'Invalid variation value',
      payload: {
        sdkKey: '8mR1pGh8u2ztUP8GqjmQq',
        userId: 'test-user-3',
        forceVariation: {
          'feature_1': 'invalid_value'
        }
      },
      // In a full implementation, we might expect 400, but for local testing we'll accept any response
      expectedStatusCode: 200
    }
  ];
...
module.exports = forcedVariationTests; 