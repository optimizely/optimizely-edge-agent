...
/**
 * Flag to determine if itty-router worker should be loaded
 * @type {boolean}
 */
const useIttyRouter = defaultSettings.useIttyRouter; // Set this to true or false based on your configuration
...
export default {
	async fetch(request, env, ctx) {
		...
		const logger = new Logger(env, 'info'); // Creates or retrieves the singleton logger instance
		...
		if (!requestIsForAsset && matchedRouteForAPI) {
			try {
				...
				logger.error('Failed to load itty-router worker:', error);
	...
	initializeCoreLogic(sdkKey, request, env, ctx, abstractionHelper) {
		...
		console.log(`Initializing core logic with SDK Key: ${sdkKey}`);
