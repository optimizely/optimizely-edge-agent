...
/**
 * Fastly-specific implementation of IRequestAdapter.
 * Adapts Fastly's Request object to the common interface.
 */
export class FastlyRequestAdapter implements IRequestAdapter {
   ...
   * @param request - The Fastly Request object.
   */
  constructor(request: Request) {
    if (!request) {
      throw new Error("Fastly Request object cannot be null or undefined.");
    }
  ...
  getUrl(): URL {
    ...
    try {
      // For Fastly Compute@Edge, we construct a URL from request.url
      return new URL(this.request.url);
    } catch (error) {
      console.error('Error parsing URL from Fastly request:', error);
