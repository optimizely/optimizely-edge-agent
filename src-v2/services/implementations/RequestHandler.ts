...
export class RequestHandler implements IRequestHandler {
  ...
  constructor(
    ...
    if (this.flagStorage) {
      ...
      if (this.requestTriggeringEnabled) {
        this.logger.info(`${this.logPrefix} RequestHandler: Automatic cleanup triggering enabled (interval: ${this.cleanupTriggerInterval}ms, probability: ${this.cleanupTriggerProbability*100}%)`);
      }
    }
  ...
  async handleRequest(requestAdapter: IRequestAdapter): Promise<ResponseResult> {
    ...
    this.logger.info(`${this.logPrefix} RequestHandler: Handling ${method} request ${requestId} for URL: ${url.toString()}`);
        ...
        if (this.apiRouter) {
          ...
          return {
            ...
            body: JSON.stringify({ 
              ...
              path: path,
              message: "The server is not configured to handle API requests. ApiRouter is missing from RequestHandler initialization."
            }),
            headers: {
              'Content-Type': 'application/json',
              'X-Implementation-Version': 'v2',
    ...
    switch (path) {
      ...
      case '/decide-all': {
        ...
        const decisions = this.decisionService.decideAll 
          ? await this.decisionService.decideAll(
              userContext,
              flagKeys,
              { 
                sdkKey,
      ...
      case '/decide-for-keys': {
        ...
        this.logger.info(`${this.logPrefix} RequestHandler [${requestId}]: Processing decide-for-keys with ${flagKeys.length} keys`);
  ...
  private async decideForFlag(
    ...
    try {
      ...
      return await this.decisionService.getDecision(
        userContext.userId,
        flagKey,
    ...
    try {
      ...
      if (this.edgeModeIntegration) {
        ...
        try {
          ...
          // Log success
          this.logger.info(`${this.logPrefix} RequestHandler [${requestId}]: Edge Mode Integration processed request successfully`);
          
          if (methodTimer) methodTimer.stop();
          return responseResult;
        } catch (error) {
          this.logger.error(`${this.logPrefix} RequestHandler [${requestId}]: Error in Edge Mode Integration, falling back to legacy implementation`, error);
      ...
      this.logger.debug(`${this.logPrefix} RequestHandler [${requestId}]: Found matching configuration`, matchingConfig);
      ...
      if (requireAuth) {
        ...
        if (!authHeader) {
          this.logger.warn(`${this.logPrefix} RequestHandler [${requestId}]: Authentication required but no auth header present`);
        ...
        if (matchingConfig.allowedRoles) {
          ...
          this.logger.debug(`${this.logPrefix} RequestHandler [${requestId}]: Role check required for: ${allowedRoles.join(', ')}`);
          
          // Mock role validation (future enhancement point)
          if (authHeader && allowedRoles.length > 0) {
            this.logger.debug(`${this.logPrefix} RequestHandler [${requestId}]: Auth header present, proceeding with role validation`);
      ...
      if (matchingConfig.transformContent && typeof result.body === 'string') {
        try {
          ...
          this.logger.info(`${this.logPrefix} RequestHandler [${requestId}]: Content transformation specified but not executed for security reasons`);
          ...
          this.logger.error(`${this.logPrefix} RequestHandler [${requestId}]: Failed to execute content transformation`, e);
        }
      }
  ...
  private extractVisitorIdFromCookies(requestAdapter: IRequestAdapter): string | null {
    ...
    const cookies: Record<string, string> = cookieHeader.split(';')
  ...
  private async findMatchingConfig(
    ...
    for (const [flagKey, decision] of Object.entries(decisions)) {
      ...
      if (!decision.enabled || !decision.variables) {
        this.logger.debug(`${this.logPrefix} RequestHandler: Skipping disabled flag or flag without variables: ${flagKey}`);
        continue;
      }
      ...
      try {
        ...
        if (typeof cdnVariationSettingsRaw === 'string') {
          try {
            ...
            this.logger.error(`${this.logPrefix} RequestHandler: Invalid JSON in cdnVariationSettings for flag ${flagKey}`, e);
            continue;
          }
        } else {
        ...
        this.logger.error(`${this.logPrefix} RequestHandler: Error parsing cdnVariationSettings for flag ${flagKey}`, e);
        continue;
      }
        ...
        if (settings.cdnExperimentURL.includes('?')) {
          ...
          for (const [key, value] of Object.entries(experimentParams)) {
            if (!(key in requestParams)) {
              this.logger.debug(
                `${this.logPrefix} RequestHandler: Parameter ${key} missing in request URL`
              );
              paramsMismatch = true;
              break;
            }
          ...
          if (paramsMismatch) {
            this.logger.debug(`${this.logPrefix} RequestHandler: Query parameter mismatch for ${settings.cdnExperimentURL}`);
            continue;
          }
        }
      }
      ...
      this.logger.info(`${this.logPrefix} RequestHandler: Match found for URL ${compareUrl}, flag: ${flagKey}, variation: ${decision.variationKey}`);
      ...
      if (matchingConfig) {
        ...
        if (userContext.attributes) {
          try {
            ...
            this.logger.warn(`${this.logPrefix} RequestHandler [${requestId}]: Failed to serialize user attributes`, error);
          }
        }
  ...
  private async createContentResponse(
    ...
    if (!cachedData || ttlValue === 0) {
      this.logger.debug(`${this.logPrefix} RequestHandler [${requestId}]: Cache miss, fetching content from ${cdnResponseURL}`);
      ...
      try {
        ...
        if (!response.ok) {
          this.logger.error(`${this.logPrefix} RequestHandler [${requestId}]: Failed to fetch content from ${cdnResponseURL}, status: ${response.status}`);
    ...
    if (matchingConfig.transformContent && typeof content === 'string') {
      try {
        ...
        if (transformCode && 
          ...
          const transformFn = new Function('content', 
            '"use strict";\n' + 
          ...
          if (typeof transformedContent === 'string') {
            ...
            this.logger.info(`${this.logPrefix} RequestHandler [${requestId}]: Content transformation applied successfully`);
            ...
            this.logger.warn(`${this.logPrefix} RequestHandler [${requestId}]: Content transformation returned non-string result`);
          ...
          this.logger.warn(`${this.logPrefix} RequestHandler [${requestId}]: Content transformation contains potentially unsafe code`);
        }
      } catch (e) {
        this.logger.error(`${this.logPrefix} RequestHandler [${requestId}]: Failed to execute content transformation`, e);
    ...
    const decision = await this.decisionService.getDecision(userContext.userId, matchingConfig.flagKey, userContext.attributes);
    
    // Add decision headers based on configuration
    if (config.returnDecisions !== false) {
      responseHeaders['X-Optimizely-Decision'] = btoa(JSON.stringify(decision));
    }
  ...
  private addDecisionHeadersToResponse(
    ...
    if (headerConfig['sdk-key'] !== false && config.sdkKey) {
      responseHeaders['X-Optimizely-SDK-Key'] = config.sdkKey;
    }
    
    // Add Visitor ID header if enabled and available
    if (headerConfig['visitor-id'] !== false && config.userId) {
      responseHeaders['X-Optimizely-Visitor-Id'] = config.userId;
    }
    ...
    if (Object.keys(decisions).length > 0 && headerConfig['decisions'] !== false) {
      const decisionsToEncode = config.trimmedDecisions ? 
        this.createTrimmedDecisions(decisions) : decisions;
        
      // Base64 encode the decisions for the header
      responseHeaders['X-Optimizely-Decision'] = btoa(JSON.stringify(decisionsToEncode));
    }
  ...
  private addCacheControlHeaders(
    ...
    if (config.cacheControl.default) {
      ...
      if (!responseHeaders['Cache-Control'] && config.cacheControl.default.browserTTL !== undefined) {
        ...
        responseHeaders['Cache-Control'] = browserTTL > 0 ? 
          `max-age=${browserTTL}` : 'no-store';
      }
  ...
  private addCookiesToResponse(
    ...
    if (cookies.length > 0) {
      ...
      // In HTTP, Set-Cookie headers need to be sent as multiple headers with the same name
      // However, our response object uses a simple key-value structure
      // So we join multiple cookies with a newline character which will be split later
      // when the actual HTTP response is constructed
  ...
  private createResponseHeaders(
    ...
    // Add standard decision-related headers
