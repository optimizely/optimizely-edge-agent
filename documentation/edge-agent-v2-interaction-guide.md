...
2.  **Valid `sdkKey`:** A valid Optimizely SDK key must be provided. The agent will not operate without it.

**If you receive "The Optimizely Edge Agent is disabled," it usually means one or both of these conditions are not met.**
...
*   **`flagKey`**: The key of the feature flag for which you want a decision.
    *   Sources:
        ...
        *   Query Parameter: `flagKey`
        *   Request Body: `flagKey`
    *   **The error "flag key parameter is required" means this was not provided or not found from these sources.**
...
*   `/api/decide-for-keys`: Get decisions for a specified list of flag keys. Accepts `sdkKey`, `userId`, `attributes`, `decideOptions`, other global flags, and requires a `flagKeys` (JSON array of strings, where each string is a flag key) in the request body.
...
*   **"The Optimizely Edge Agent is disabled"**:
    *   Missing or incorrect `X-Optimizely-Enable-FEX: true` header.
    *   Missing or invalid `sdkKey` (from header, query, or body).
*   **"/api/decide" returns "flag key parameter is required"**:
    *   `flagKey` was not provided in the request body (as `flagKey`), as a query parameter (as `flagKey`), or as a header (`X-Optimizely-Flag-Key`).
